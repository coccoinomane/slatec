            ...
            )
{
  ...
  *M2MAX = MIN(L2,L3-M1);
  ...
  if (L1-fabs(M1)+EPS < ZERO) {
    *IER=1;
    printf ("ERROR, %s: L1-ABS(M1) less than zero or L1+fabs(M1) not integer.\n", __func__);
    return _FAILURE_;
  }
  else if ((L1+L2-L3 < -EPS) || (L1-L2+L3 < -EPS) || (-L1+L2+L3 < -EPS)) {
    *IER=2;
    printf ("ERROR, %s: L1, L2, L3 do not satisfy triangular condition.\n", __func__);
    return _FAILURE_;
  }
  ...
  *IER=5;
  printf ("ERROR, %s: M2MIN greater than M2MAX.\n", __func__);
  return _FAILURE_;
  ...
  _21_:
    *IER=6;
    printf ("ERROR, %s: Dimension of result array for 3j coefficients too small.\n", __func__);
    return _FAILURE_;
            ...
            )
{
  ...
  *IER = 0;
  ...
  if ( (L2-fabs(M2)+EPS < ZERO) || (L3-fabs(M3)+EPS < ZERO)) {
    *IER=1;
    printf ("ERROR, %s: L2-ABS(M2) or L3-fabs(M3) less than zero.\n", __func__);
    return _FAILURE_;
  }
  else if ((fmod(L2+fabs(M2)+EPS,ONE) >= EPS+EPS) || (fmod(L3+fabs(M3)+EPS,ONE) >= EPS+EPS)) {
    *IER=2;
    printf ("ERROR, %s: L2+ABS(M2) or L3+fabs(M3) not integer.\n", __func__);
    return _FAILURE_;
  }
  ...
  *L1MAX = L2 + L3;
  ...
  if ( fmod(*L1MAX-*L1MIN+EPS,ONE) >= EPS+EPS) {
    *IER=3;
    printf ("ERROR, %s: *L1MAX-*L1MIN not integer.\n", __func__);
    return _FAILURE_;
  }
  ...
  *IER=4;
  printf ("ERROR, %s: L1MIN greater than L1MAX.\n", __func__);
  return _FAILURE_;
           ...
           )
{
  ...
  *IER = 0;
  ...
  if ( (fmod(L2+L3+L5+L6+EPS,ONE) >= EPS+EPS) || (fmod(L4+L2+L6+EPS,ONE) >= EPS+EPS)) {
    *IER=1;
    printf ("ERROR, %s: L2+L3+L5+L6 or L4+L2+L6 not integer.\n", __func__);
    return _FAILURE_;
  }
  else if((L4+L2-L6 < ZERO) || (L4-L2+L6 < ZERO) || (-L4+L2+L6 < ZERO)) {
    *IER=2;
    printf ("ERROR, %s: L4, L2, L6 triangular condition not satisfied.\n", __func__);
    return _FAILURE_;
  }
  else if ((L4-L5+L3 < ZERO) || (L4+L5-L3 < ZERO) || (-L4+L5+L3 < ZERO)) {
    *IER=3;
    printf ("ERROR, %s: L4, L5, L3 triangular condition not satisfied.\n", __func__);
    return _FAILURE_;
  }
  ...
  *IER=5;
  ...
  _21_:
    *IER=6;
    printf ("ERROR, %s: Dimension of result array for 6j coefficients too small.\n", __func__);
    return _FAILURE_;
